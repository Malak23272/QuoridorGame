...
inline int Minimax ::minimaxRecursive(Board currentBoard, Player currentPlayer, Player opponentPlayer, int depth, int alpha, int beta, bool isMaximizing) {
    ...
    if (isMaximizing) {
        ...
        for (auto const& m : moves) {
            ...
            int current_score =
                minimaxRecursive(tempBoard, tempCurrent, tempOpponent, depth - 1, alpha, beta, !isMaximizing);
    ...
    if (activePlayer.getWallsRemaining() > 0) {
        ...
        for (int x = x0; x <= x1; ++x) {
            for (int y = y0; y <= y1; ++y) {
                ...
                hwall.topLeft.x = x;
                hwall.topLeft.y = y;
                if (board.isValidWallPlacement(hwall) &&
                    !board.doesWallBlockPath(hwall, activePlayer, opponnentPlayer)) {
                ...
                Vwall.topLeft.x = x;
                Vwall.topLeft.y = y;
                if (board.isValidWallPlacement(Vwall) &&
                    !board.doesWallBlockPath(Vwall, activePlayer, opponnentPlayer)) {
