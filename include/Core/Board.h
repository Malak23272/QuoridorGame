...
private:
        ...
        if ((to.x == from.x + 1) && to.y == from.y) {
            ...
            for (int i = 0; i < placedWalls.size(); ++i) {
                if (placedWalls[i].orientation == WallOrientation::VERTICAL) {
                    ...
                    else if ((placedWalls[i].topLeft.x == from.x) && (placedWalls[i].topLeft.y == from.y - 1))
                        return true;
                }
            }
            ...
            for (int i = 0; i < placedWalls.size(); ++i) {
                if (placedWalls[i].orientation == WallOrientation::HORIZONTAL) {
                    if ((placedWalls[i].topLeft.x == from.x) && (placedWalls[i].topLeft.y == from.y)) return true;
                    else if ((placedWalls[i].topLeft.x == from.x - 1) && (placedWalls[i].topLeft.y == from.y))
                        return true;
                }
            }
        }
    ...
    bool hasPath_to_Goal(Player player) {
        ...
        while (!positions.empty()) {
            ...
            if (pos.y == player.getGoalRow()) {  // if yes so The player found the Goal
                return true;
            }
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if ((p.x < 9 && p.y < 9 && p.y >= 0 && p.x >= 0) && (visited[p.x][p.y] == false) &&
                    !(isEdgeBlocked(pos, p))) {
...
public:
 ...
 int getShortestPath(Player& player, Position blockedCell);
