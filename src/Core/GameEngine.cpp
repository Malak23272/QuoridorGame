...
bool GameEngine::saveGame(const std::string& filename) {
    ...
    outFile << player1.getPosition().x << " " << player1.getPosition().y << " " << player1.getWallsRemaining() << "\n";

    // 3. Save Player 2 Data
    outFile << player2.getPosition().x << " " << player2.getPosition().y << " " << player2.getWallsRemaining() << "\n";

    // 4. Save Board Walls
    const auto& walls = board.getWalls();
    outFile << walls.size() << "\n";  // Print wall count first
    for (const auto& wall : walls) {
        outFile << wall.topLeft.x << " " << wall.topLeft.y << " " << static_cast<int>(wall.orientation) << "\n";
    }
